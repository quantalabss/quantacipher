...
import { get_wasm_version, } from 'quantacipher-wasm';
// ============================================================
...
export class QuantaCipher {
     ...
     */
    async vaultData(plaintext, metadata = {}) {
     ...
     */
    encryptSecure(plaintext, publicKeyB64) {
     ...
     * Requires WASM rebuild.
     */
    decryptSecure(ciphertextPayload, privateKeyB64) {
     ...
     */
    async secureData(plaintext, publicKeyB64, metadata = {}) {
    ...
    // GATEWAY
    // ----------------------------------------------------------
    async sendToGateway(ciphertext, metadata = {}) {
        console.log(`[QuantaCipher SDK] Tunneling to Gateway → ${this.gatewayUrl}`);
        try {
            const response = await axios.post(this.gatewayUrl, { ciphertext, metadata, timestamp: Date.now() }, { headers: { 'x-api-key': this.apiKey, 'Content-Type': 'application/json' } });
    ...
    // UTILITIES
    // ----------------------------------------------------------
    getVersion() { return get_wasm_version(); }
    isDualModeAvailable() { return this.dualModeAvailable; }
    requireDualMode(methodName) {
        if (!this.dualModeAvailable) {
            throw new Error(`[QuantaCipher SDK] ${methodName}() requires the dual-mode WASM build.\n` +
                `Rebuild in WSL:\n  cd /mnt/e/temp/quantacipher-wasm\n  wasm-pack build --target web --out-dir pkg\n` +
                `Then reinstall: cd ../quantacipher-sdk-js && npm install`);
        }
    }
    /** @deprecated Use vaultData() instead */
    async secureDataLegacy(plaintext, metadata = {}) {
