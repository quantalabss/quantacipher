...
NIST ML-KEM-1024 (FIPS 203) · FN-DSA / ML-DSA Signing · AES-256-GCM · WebAssembly</p>

[![npm version](https://img.shields.io/npm/v/@quantalabss/quantacipher-sdk?color=C4ED5F&style=flat-square)](https://www.npmjs.com/package/@quantalabss/quantacipher-sdk)
[![npm downloads](https://img.shields.io/npm/dm/@quantalabss/quantacipher-sdk?style=flat-square&color=white)](https://www.npmjs.com/package/@quantalabss/quantacipher-sdk)
...
[![Powered by WASM](https://img.shields.io/badge/powered%20by-WebAssembly-654FF0?style=flat-square)](https://www.npmjs.com/package/@quantalabss/quantacipher-wasm)
...
> Seal data forever. No decryption possible. Designed for permanent tamper-evident records, compliance event logs, and audit timestamps where data must be proven to exist but never read back.

**How it works:**
1. An ephemeral ML-KEM-1024 keypair is generated locally inside the WASM engine
...
console.log(receipt.encryptionScheme); // "ML-KEM-1024 (FIPS 203) + AES-256-GCM"
...
// keys.publicKey  → base64 ML-KEM-1024 public key  (safe to share)
// keys.privateKey → base64 ML-KEM-1024 private key (NEVER share — store securely)
// keys.algorithm  → "ML-KEM-1024"
// keys.version    → "1.0"
...
// → throws QuantaCipher Error: ML-KEM decapsulation failed (wrong key?)
```
...
> Sign documents and payloads with NIST-standard post-quantum digital signatures.
> The algorithm is embedded in the signature — `verifySignature()` never needs to know which was used at sign time.
...
// Generate a signing keypair (default: Falcon-512 / FN-DSA FIPS 206 draft)
const kp = signer.generateSigningKeyPair();
// Or explicitly: signer.generateSigningKeyPair(AlgorithmId.ML_DSA_44)

// Sign any string or Uint8Array — handles payloads of any size
const sig = signer.signPayload('My compliance document', kp.private_key);
// → { alg: "falcon-512-fips206-draft", sig: "<base64>", ver: 1 }

// Verify — alg is read from the envelope, no caller-side tracking needed
const valid = signer.verifySignature('My compliance document', sig, kp.public_key);
// → true

// Store or transmit the signature as JSON
const json = signer.envelopeToJson(sig);
const restored = signer.envelopeFromJson(json); // throws if alg/sig/ver missing
```

> A missing or unrecognized `alg` field always throws — never silently falls back.
> See [SECURITY.md](./SECURITY.md) for known limitations (Falcon-512 domain separation).
...
| Method | Description |
|---|---|
| `generateKeypair()` | Generate an ML-KEM-1024 keypair locally |
...
**`QuantaCipherSign` methods** (v2):

| Method | Description |
|---|---|
| `generateSigningKeyPair(algorithm?)` | Generate PQ signing keypair |
| `signPayload(payload, privateKeyB64, options?)` | Sign a string or Uint8Array |
| `verifySignature(payload, signature, publicKeyB64)` | Verify — reads alg from envelope |
| `envelopeToJson(envelope)` | Serialize `SignatureEnvelope` to JSON string |
| `envelopeFromJson(json)` | Deserialize + validate envelope fields |
...
```
QZ_VAULT_V1  : <base64 ML-KEM-1024 ciphertext> : <base64 AES-GCM nonce> : <base64 AES-GCM ciphertext>
QZ_SECURE_V1 : <base64 ML-KEM-1024 ciphertext> : <base64 AES-GCM nonce> : <base64 AES-GCM ciphertext>
```
