...
NIST ML-KEM (Kyber-1024) · AES-256-GCM · Compiled from Rust · Dual-Mode</p>

[![npm version](https://img.shields.io/npm/v/quantacipher-wasm?color=C4ED5F&style=flat-square)](https://www.npmjs.com/package/quantacipher-wasm)
[![npm downloads](https://img.shields.io/npm/dm/quantacipher-wasm?style=flat-square&color=white)](https://www.npmjs.com/package/quantacipher-wasm)
...
> **Most developers should use [`quantacipher-sdk`](https://www.npmjs.com/package/quantacipher-sdk)** — the high-level TypeScript SDK that wraps this package and adds the gateway integration, dual-mode abstraction, and receipt handling. Use `quantacipher-wasm` directly only if you need the raw WASM engine without the SDK layer (e.g., in a browser app with a custom architecture, or a non-Node.js runtime).
...
// → Error: QuantaCipher Error: Kyber decapsulation failed (wrong key?)
```
...
| `generate_keypair` | `() => string` | Generate Kyber-1024 keypair, returns JSON string |
...
```
QZ_VAULT_V1  : <base64 Kyber-1024 CT> : <base64 12-byte nonce> : <base64 AES-256-GCM CT>
QZ_SECURE_V1 : <base64 Kyber-1024 CT> : <base64 12-byte nonce> : <base64 AES-256-GCM CT>
```
...
| Property | Value |
|---|---|
| KEM Algorithm | CRYSTALS-Kyber-1024 (NIST FIPS 203 / ML-KEM) |
...
| Package | Description |
|---|---|
| [`quantacipher-sdk`](https://www.npmjs.com/package/quantacipher-sdk) | High-level TypeScript SDK (recommended for most users) |
